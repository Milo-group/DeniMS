...
class ExtraMolecularFeatures:
    def __init__(self, dataset_infos, embeddings = False):
    ...
    def __call__(self, noisy_data, embeddings = None):
        ...
        if self.embeddings and embeddings is not None:
            return utils.PlaceHolder(X=torch.cat((charge, valency), dim=-1), E=extra_edge_attr, y=torch.cat((weight, embeddings), dim=-1))
