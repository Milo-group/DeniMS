...
class EigenFeatures:
    ...
    def _debug_batch_error(self, L, A, mask, noisy_data, error, op_name):
        """Debug information when eigendecomposition fails."""
        import os
        import torch
        
        print(f"\n{'='*80}")
        print(f"ERROR in {op_name}: {error}")
        print(f"{'='*80}")
        ...
        for b_idx in range(batch_size):
            L_b = L[b_idx].cpu()
            A_b = A[b_idx].cpu()
            mask_b = mask[b_idx].cpu()
            
            # Check for NaN/Inf
            has_nan = torch.isnan(L_b).any().item()
            has_inf = torch.isinf(L_b).any().item()
            
            # Compute condition number
            try:
                cond_num = torch.linalg.cond(L_b).item()
            except:
                cond_num = float('inf')
            ...
            eigenvals_try = None
            try:
                eigenvals_try = torch.linalg.eigvalsh(L_b)
                min_eigval = eigenvals_try.min().item()
                max_eigval = eigenvals_try.max().item()
            except:
                min_eigval = None
                max_eigval = None
            ...
            num_edges = (A_b > 0).sum().item() // 2  # Undirected, so divide by 2
            
            # Check if this is the problematic batch
            is_problematic = has_nan or has_inf or cond_num > 1e12 or (min_eigval is not None and min_eigval < -1e-5)
            
            if is_problematic or b_idx == 4:  # Always print batch 4 (the one that failed)
                print(f"\nBatch {b_idx}:")
                ...
                if min_eigval is not None:
                    ...
                    print(f"  - Max eigenvalue: {max_eigval:.2e}")
                print(f"  - L matrix stats: min={L_b[mask_b.unsqueeze(1) * mask_b.unsqueeze(0) > 0].min():.2e}, "
                      f"max={L_b[mask_b.unsqueeze(1) * mask_b.unsqueeze(0) > 0].max():.2e}, "
                      f"mean={L_b[mask_b.unsqueeze(1) * mask_b.unsqueeze(0) > 0].mean():.2e}")
                
                # Save problematic batch to file
                debug_dir = "debug_eigenfeatures"
                ...
                torch.save({
                    ...
                    'num_nodes': num_nodes,
                    'num_edges': num_edges,
                }, os.path.join(debug_dir, f"problematic_batch_{b_idx}.pt"))
                print(f"  - Saved debug info to {debug_dir}/problematic_batch_{b_idx}.pt")
