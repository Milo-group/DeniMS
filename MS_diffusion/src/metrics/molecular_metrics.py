...
class SamplingMolecularMetricsEdges(nn.Module):
    ...
    def forward(self, molecules: list, name, current_epoch, val_counter, local_rank, test=False, targets = None):
        ...
        if wandb.run:
            ...
            wandb.log({'basic_metrics/edge_mae': edge_mae}, commit=False)

        if local_rank == 0:
            print("Custom metrics computed.")
        if local_rank == 0:
            valid_unique_molecules = rdkit_metrics[1]
            with open(f'graphs/{name}/valid_unique_molecules_e{current_epoch}_b{val_counter}.txt', "w") as textfile:
                for molecule in valid_unique_molecules:
                    textfile.write(molecule + "\n")
            print("Stability metrics:", stability, "--", rdkit_metrics[0])

        return all_smiles, identical_list, tanimoto_list, mces_list

    def reset(self):
        for metric in [self.edge_dist_mae]:
            metric.reset()
