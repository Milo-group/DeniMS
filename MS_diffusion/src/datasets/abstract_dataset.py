...
class AbstractDataModule(LightningDataset):
    ...
    def node_counts(self, max_nodes_possible=300):
        ...
        for loader in [self.train_dataloader(), self.val_dataloader()]:
            for data in tqdm(loader, desc="node counts"):
    ...
    def node_types(self):
        ...
        for i, data in tqdm (enumerate(self.train_dataloader()), desc="node types"):
    ...
    def edge_counts(self):
        ...
        for i, data in tqdm (enumerate(self.train_dataloader()), desc="edge counts"):
            unique, counts = torch.unique(data.batch, return_counts=True)
...
class AbstractDatasetInfos:
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features, embeddings = False):
        ...
        if embeddings:
            ex_extra_molecular_feat = domain_features(example_data, embeddings = example_batch.embedding)
        ...
        self.output_dims = {'X': example_batch['x'].size(1),
                            'E': example_batch['edge_attr'].size(1),
                            'y': example_batch.embedding.size(1) if embeddings else self.input_dims['y']}  # + 1 due to time conditioning

        self.embeddings_dims = example_batch.embedding.size(1) if embeddings else 0
